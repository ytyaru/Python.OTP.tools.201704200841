    ...
    def __OpenDb(self):
        ...
        if 0 < self.account['Accounts'].count():
            ...
            if not(os.path.isfile(self.__files['license'])):
                ...
                account = self.account['Accounts'].find().next()
                twofactor = self.account['TwoFactors'].find_one(AccountId=account['Id'])
                authentications = []
...
#                self.account['AccessTokens'].find_one(Scopes!=None)
...
#                authData.Load(self, self.account['Accounts'].find().next()['Username'])
#                client = web.service.github.api.v3.Client.Client(self, authData)
...
#            user = web.service.github.api.v3.CurrentUser.CurrentUser(self, username)
            authData = web.service.github.api.v3.AuthenticationData.AuthenticationData()
            authData.Load(self.account, username)
            # ダミー引数を渡す
...
#            client = web.service.github.api.v3.Client.Client(self, authData, repo)
            account = self.account['Accounts'].find_one(Username=username)
            twofactor = self.account['TwoFactors'].find_one(AccountId=account['Id'])
            authentications = []
            if None is not twofactor:
                authentications.append(TwoFactorAuthentication(account['Username'], account['Password'], twofactor['Secret']))
                ...
                authentications.append(BasicAuthentication(account['Username'], account['Password']))
            client = web.service.github.api.v3.Client.Client(self, authentications)
#            inserter = database.src.repo.insert.Main.Main(self, client, authData, repo)
