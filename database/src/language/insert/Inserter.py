...
class Inserter:
    def __init__(self, db_path):
        self.__db_lang = dataset.connect('sqlite:///' + db_path)

    def Insert(self, y):
        self.__db_lang.begin()
        count = 0
        for key in y.keys():
            print(key)
            print(y[key])
            if None is self.__db_lang['Languages'].find_one(Key=key):
                self.__db_lang['Languages'].insert(self.__CreateLanguages(key, y[key]))
                language_id = self.__db_lang['Languages'].find_one(Key=key)['Id']
                ...
                count += 1
                print('重複レコード。')
                print(self.__db_lang['Languages'].find_one(Key=key))
        self.__db_lang.commit()
        print(count)
    ...
    def __InsertAliases(self, language_id, lang):
        ...
        for aliase in lang['aliase']:
            self.__db_lang['Aliases'].insert(dict(
    ...
    def __InsertExtensions(self, language_id, lang):
        ...
        for extension in lang['extensions']:
            self.__db_lang['Extensions'].insert(dict(
                LanguageId=language_id,
                Extension=extension,
    ...
    def __InsertFileNames(self, language_id, lang):
        ...
        for filename in lang['filenames']:
            self.__db_lang['FileNames'].insert(dict(
                LanguageId=language_id,
                FileName=filename
            ))
    def __InsertInterpreters(self, language_id, lang):
        ...
        for interpreter in lang['interpreters']:
            self.__db_lang['Interpreters'].insert(dict(
                LanguageId=language_id,
